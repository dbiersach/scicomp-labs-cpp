...
void draw(SimpleScreen& ss)
{
    ...
    for (double phi = 0; phi < M_PI; phi += deltaPhi)
    {
        ...
        double rA = radius; // = radius * sin(phi);
        double rB = radius; // = radius * sin(phi + deltaPhi);
        ...
        for (double theta = 0; theta < M_PI * 2; theta += deltaTheta)
        {
            ...
            ss.DrawFacet(f, al_map_rgb(0, 0, 0), al_map_rgb(0, 0, 255), 1, false, false, 1);
        }
    }
}
