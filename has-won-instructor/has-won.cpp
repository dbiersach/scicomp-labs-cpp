...
bool HasWon(int player)
{
    ...
    if (HasChain(player, 3, 4, 5)) return true;
    if (HasChain(player, 6, 7, 8)) return true;

    // Check Columns
    if (HasChain(player, 0, 3, 6)) return true;
    if (HasChain(player, 1, 4, 7)) return true;
    if (HasChain(player, 2, 5, 8)) return true;

    // Check Diagonals
    if (HasChain(player, 0, 4, 8)) return true;
    if (HasChain(player, 2, 4, 6)) return true;
